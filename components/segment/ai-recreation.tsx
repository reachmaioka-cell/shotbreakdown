...
import { StoredAiRecreationSchema, type StoredAiRecreation } from "@/lib/validation";

export type AiRecreationStatusValue = "pending" | "ready" | "failed" | "missing" | null;
...
const FEASIBILITY = {
  ...
  difficult: {
    label: "Difficult",
    tone: "accent",
    title: "Expect many attempts and hand fixes, and a result that is close rather than right",
  },
  "not-yet": {
    label: "Not yet",
    tone: "danger",
...
export function AiRecreation({
  ...
  const [recreation, setRecreation] = useState<StoredAiRecreation | null>(initialRecreation);
  ...
  if (
    ...
    setSeen({ status: initialStatus, recreation: initialRecreation, error: initialError });
  ...
  async function generate() {
    ...
    setActionError(null);
    try {
      const res = await fetch(`/api/videos/${videoId}/ai-recreation`, { method: "POST" });
      if (res.status === 401) {
        toLogin();
        return;
      }
      ...
      if (!res.ok) {
        setActionError(data.error ?? "Could not start the AI guide. Try again.");
        return;
      }
      setError(null);
      setPollError(null);
              ...
              <dl className="grid max-w-[72ch] grid-cols-[minmax(0,9rem)_minmax(0,1fr)] gap-x-4 gap-y-1.5">
                {recreation.tools.map((tool, i) => (
                  <div key={i} className="contents">
                    <dt className="text-[13px] leading-relaxed text-text-0">{tool.name}</dt>
                    <dd className="text-[13px] leading-relaxed text-text-2">{tool.role}</dd>
                  </div>
                ))}
              </dl>
            </div>
            ...
            <div>
              ...
              <ul className="flex flex-col gap-3">
                {recreation.prompts.map((prompt, i) => (
                  <li key={i} className="rounded-[3px] border border-line bg-ink-1 p-3">
      ...
      {status === "pending" ? (
        <div role="status" className="rounded-[3px] border border-line bg-ink-1 p-4">
          ...
          <p className="mt-2 text-[12px] text-text-3">
            You can leave this page. The guide keeps writing and will be here when you come back.
          </p>
        ...
        <div role="alert" className="rounded-[3px] border border-danger/30 bg-danger/5 p-4">
          <p className="text-[13px] text-text-0">The AI route could not be written.</p>
        ...
        <div className="rounded-[3px] border border-dashed border-line p-4">
          ...
          <p id={noteId} className="mt-2 max-w-[72ch] text-[12px] leading-relaxed text-text-2">
            {hasBreakdown
