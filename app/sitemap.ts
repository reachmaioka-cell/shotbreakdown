...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const taxonomyUrls: MetadataRoute.Sitemap = TAXONOMY_GROUPS.flatMap((group) =>
    group.entries
      .filter((entry) => {
  ...
  const tagUrls: MetadataRoute.Sitemap = (facets.tags ?? [])
  ...
  const learn: MetadataRoute.Sitemap = LEARN_TOPICS.map((topic) => ({
